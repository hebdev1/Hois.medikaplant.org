...
// ───────────────────────────────────────────────────────────────────────────
// Patient list — the original /admin/health table view, lifted into its
// own component so the unified tab page can pull it in without inlining
// everything. The filter form posts back to /admin/health with the same
// `tab=patients` so the active tab survives a filter submit.
// ───────────────────────────────────────────────────────────────────────────
...
export default async function PatientsView({
  q,
  condition,
}: {
  q?: string;
  ...
  const [profilesResult, medicalResult, logsResult, treatmentsResult] =
    await Promise.all([
      ...
      supabase
        ...
        .limit(2000),
      supabase.from('treatment_recommendations').select('user_id, status'),
    ]);
  ...
  const conditionFilter = condition?.toLowerCase();
  const queryFilter = q?.toLowerCase().trim();
  ...
  return (
    <>
      ...
      <form className="mb-5 flex flex-wrap items-center gap-2" action="/admin/health">
        ...
        {(q || condition) && (
          <Link
            href="/admin/health?tab=patients"
